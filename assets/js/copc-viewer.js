...
let _rampLut = null;
// Ramp for colourless / B&W clouds — 256 RGB buckets (3 floats each). "Ardoise":
// a cool light grey, dark→light, with a LIGHT floor so nothing reads near-black.
// The value blends height + facade-depth (see loadNode), a smoothstep curve adds
// contrast, and everything uploads in LINEAR space (renderer output-encodes).
function rampLut() {
  if (_rampLut) return _rampLut;
  const stops = [
    [0.00, 0.52, 0.57, 0.63],   // shadows — cool light grey
    [0.50, 0.75, 0.80, 0.86],   // mid
    [1.00, 0.97, 0.99, 1.00],   // highlights — near-white
  ];
  const sample = (t, ch) => {
...
async function initCopc(container) {
  ...
  let grayscale = false;          // B&W scan → render the grey/Ardoise ramp
  ...
  async function loadNode(key, node) {
    ...
    try {
      ...
      // Colourless cloud (no RGB at all, or detected B&W) → Ardoise grey ramp
      // that blends HEIGHT with the facade-DEPTH axis, so the form a flat colour
      // would lose comes back as tonal relief. RGB clouds read their own LUT.
      ...
      for (let i = 0; i < n; i++) {
        ...
        if (ramp) {
          ...
          // detail; height (0.4) keeps an overall vertical read.
